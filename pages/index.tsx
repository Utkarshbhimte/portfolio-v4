...
export const getStaticProps: GetStaticProps<AppProps> = async () => {
  ...
  const siteContent = siteContentTableData.reduce((total, curr) => ({ ...total, [curr.key]: curr.value }), {})

  return {
    props: {
      achievements,
      posts: postResponse.data.user.publication.posts.sort((a, b) => a.coverImage.length - b.coverImage.length),
      siteContent,
